...
bool CLanServer::DisConnect(UINT64 SessionID)
{
	...
	if (m_pSessionArray[StackIndex].IsUseSession == FALSE)
	{
		...
		//OnError(&Error);
		WriteError(WSAGetLastError(), SERVER_ERR::SESSION_NULL_ERR);
		return false;
		//////////////////////////////////////////////////////
	}
...
char CLanServer::RecvPost(Session *pSession)
{
	...
	if (retval == SOCKET_ERROR)
	{
		...
		if (GetLastErr != ERROR_IO_PENDING)
		{
			...
			//OnError(&Error);
			WriteError(WSAGetLastError(), SERVER_ERR::WSARECV_ERR);
			return POST_RETVAL_ERR;
		}
	}
	...
	while (1)
	{
		...
		if (retval == SOCKET_ERROR)
		{
			...
			if (err != ERROR_IO_PENDING)
			{
				...
				//OnError(&Error);
				WriteError(WSAGetLastError(), SERVER_ERR::WSASEND_ERR);
...
void CLanServer::WriteError(int WindowErr, int UserErr)
{
	st_Error Error;
