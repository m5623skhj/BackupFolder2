...
public :
	...
	// 해당 유저에게 패킷을 전송 한 이후에 접속을 끊음
	void SendPacketAndDisConnect(CNetServerSerializationBuf *pSendPacket);
	// 해당 유저의 접속을 끊음
	void DisConnect();
...
protected :
	...
	// 인증 스레드에서 하트비트 처리됨
	virtual void OnAuth_ClientLeaveHeartbeat() = 0;
	// 게임 스레드에서 하트비트 처리됨
	virtual void OnGame_ClientLeaveHeartbeat() = 0;
...
private :
	...
	// 외부에서 보내고 끊기를 사용할 경우 플래그를 true 로 변경함
	bool													m_bSendAndDisConnect;
	...
	AcceptUserInfo											m_UserNetworkInfo;
